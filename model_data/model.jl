...
model = Model()
set_optimizer(model, Gurobi.Optimizer)
@variable(model, g_gen[g in 1:2, t in Time] >= 0)
@variable(model, w_tur[          t in Time] >= 0)
@variable(model, w_lev[          t in union(0,Time)] >= 0)
@variable(model, w_spi[          t in Time] >= 0)

@objective(model, Min, sum(g_gen[g,t]*Opex[g] for g in 1:2, t in Time)   )

@constraint(model,Demand[t in Time], sum(g_gen[g,t] for g in 1:2) == dem[t] ) 

@constraint(model, PowerLimit[g in 1:2, t in Time], L_lim[g] <= g_gen[g,t] <= U_lim[g] )

@constraint(model, Reservoir[t in Time], w_lev[t] == w_lev[t-1] - w_tur[t] + w_apo - w_spi[t])
...
for t in Time
    g_gen[1,t] = piecewiselinear(model, w_tur[t], w_lev[t-1], q_range, v_range, p)#,method=:ZigZag)
end


#@constraint(model, WatTurb[t in Time], g_gen[1,t] == 9.81*w_tur[t]*eff*0 )
@constraint(model, WaterLimit[ t in Time], w_tur_m <= w_tur[t] + w_spi[t] <= w_tur_M )
...
if termination_status(model) == OPTIMAL
    for t in Time
        ...
        Gen[1][t]   = round(JuMP.value(g_gen[1,t]) , digits = 2)
